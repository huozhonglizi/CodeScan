...
class MsgManager extends Handler implements Comparator<AppMsg> {
	...
	/**
	 * @return A {@link MsgManager} instance to be used for given
	...
	void add(AppMsg appMsg) {
		...
		if (appMsg.mInAnimation == null) {
			appMsg.mInAnimation = AnimationUtils.loadAnimation(
					appMsg.getActivity(), android.R.anim.fade_in);
		}
		if (appMsg.mOutAnimation == null) {
			appMsg.mOutAnimation = AnimationUtils.loadAnimation(
	...
	static void obtainShowing(Collection<AppMsg> from,
			Collection<AppMsg> appendTo) {
		for (AppMsg msg : from) {
			if (msg.isShowing()) {
				appendTo.add(msg);
			}
	...
	private void displayMsg() {
		...
		if (!appMsg.isShowing()) {
			...
			sendMessageDelayed(msg,
					appMsg.getDuration() + appMsg.mInAnimation.getDuration()
							+ appMsg.mOutAnimation.getDuration());
		}
	}
	...
	private void removeMsg(final AppMsg appMsg) {
		...
		if (parent != null) {
			appMsg.mOutAnimation.setAnimationListener(new OutAnimationListener(
					appMsg));
			view.clearAnimation();
	...
	private static class OutAnimationListener implements
		...
		public void onAnimationEnd(Animation animation) {
			...
			if (appMsg.isFloating()) {
				...
				if (parent != null) {
					parent.post(new Runnable() { // One does not simply
													// removeView
						@Override
						public void run() {
